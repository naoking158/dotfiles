...
- [[#general-configuration][General Configuration]]
  ...
  - [[#wayland-clipboard-bridge][Wayland clipboard bridge]]
  ...
  (leaf cus-start
    ...
    (defun general-init-hook nil
      ...
      (if-let* ((gls (executable-find "gls")))
        ...
        ;; macOS BSD ls fallback: --dired and --group-directories-first
        ;; are unsupported, so disable --dired emission and use safe switches.
...
:PROPERTIES:
:ID:       A3B7C291-5E4F-4D82-B9C3-1F6E8D0A2C47
:END:

pgtk + Wayland + wl-clipboard が揃った環境でのみ、
interprogram-cut-function / interprogram-paste-function を
wl-copy / wl-paste でオーバーライドし、cosmic-comp の
XWayland → Wayland clipboard 同期不全を回避する。

#+begin_src emacs-lisp
(leaf *wayland-clipboard-bridge
  :leaf-defer nil
  :when (and (featurep 'pgtk)
             (executable-find "wl-copy")
             (executable-find "wl-paste"))
  :preface
  (defvar my/wl-copy-last nil
    "Last text sent via `my/wl-copy'. Used to suppress double yank.")

  (defun my/wl-copy (text)
    "Send TEXT to Wayland CLIPBOARD via wl-copy (async)."
    (let ((default-directory temporary-file-directory)
          (process-connection-type nil))
      (let ((proc (start-process "wl-copy" nil "wl-copy")))
        (process-send-string proc text)
        (process-send-eof proc)
        (setq my/wl-copy-last text)
        proc)))

  (defun my/wl-paste ()
    "Read Wayland CLIPBOARD via wl-paste. Return nil if empty or own-copy."
    (let ((default-directory temporary-file-directory))
      (let ((text (with-output-to-string
                    (with-current-buffer standard-output
                      (process-file "wl-paste" nil '(t nil) nil
                                    "-n" "--no-newline")))))
        (cond
         ((or (null text) (string-empty-p text)) nil)
         ((and my/wl-copy-last (string= text my/wl-copy-last)) nil)
         (t text)))))
  :config
...
(leaf themes
  ...
  (leaf standard-themes
    ...
    (defun my/load-standard-theme (sym-theme)
      ...
      (pcase sym-theme
        ...
        ('standard-light-tinted (load-theme 'standard-light-tinted :no-confirm))
        )
  ...
  (defun my/default-theme nil
    ...
    ;; (my/load-theme 'standard-light)
    (my/load-theme 'standard-light-tinted)
    )
...
(leaf treesit
  ...
  :config
  ;; Grammars are provided by Nix via `programs.emacs.extraPackages`
  ;; (`treesit-grammars.with-grammars`). The emacsWithPackages wrapper writes
  ;; an `(add-to-list 'treesit-extra-load-path ...)` into site-start.el, so no
  ;; manual setup is required here.
  ...
  ;; Native tree-sitter language injection for GraphQL in TSX/TypeScript
  ;; Replaces polymode/mmm-mode approaches with Emacs 30 built-in functionality
  (leaf *ts-graphql-injection
    :doc "Enable GraphQL syntax highlighting in TSX/TypeScript gql tagged templates"
    :after graphql-ts-mode
    :config
    (defun my/setup-graphql-injection ()
      "Enable GraphQL syntax highlighting inside gql` ... ` blocks in TSX/TypeScript."
      (when (treesit-language-available-p 'graphql)
        ;; Determine host language from major mode
        (let ((host-lang (pcase major-mode
                           ('tsx-ts-mode 'tsx)
                           ('typescript-ts-mode 'typescript)
                           (_ 'tsx))))

          ;; 1. Set primary parser (Emacs 30 recommended)
          (setq-local treesit-primary-parser (treesit-parser-create host-lang))

          ;; 2. Create GraphQL parser for this buffer
          (treesit-parser-create 'graphql)

          ;; 3. Set up range rules WITH :offset to skip backticks
          (setq-local treesit-range-settings
                      (treesit-range-rules
                       ...
                       '((call_expression
                          ...
                          arguments: (template_string) @capture
                          (:match "^gql$\\|^graphql$" @fn)))))

          ;; 4. Define GraphQL font-lock rules WITH :override t
          (let ((graphql-rules-with-override
                 (treesit-font-lock-rules
                  ...
                  '(["query" "mutation" "subscription" "fragment" "on"
                     "type" "input" "enum" "scalar" "interface" "union"
                     "extend" "implements" "directive" "schema" "repeatable"]
                  ...
                  :override t
                  :feature 'type
                  '([(type) (named_type)] @font-lock-type-face)
                  
                  :language 'graphql
                  :feature 'variable
                  '([((variable) @font-lock-variable-name-face)
                     (field (name) @font-lock-variable-name-face)
                     (field_definition (name @font-lock-variable-name-face))
                     (input_value_definition (name) @font-lock-variable-name-face)
                     (argument (name) @font-lock-variable-name-face)
                     (object_field (name) @font-lock-variable-name-face)])

                  :language 'graphql
                  :override t
                  :feature 'string
                  '([(string_value) (description)] @font-lock-string-face)
                  ...
                  :override t
                  :feature 'constant
                  '([(boolean_value) (null_value) (directive_location)]
                    @font-lock-constant-face)

                  :language 'graphql
                  :feature 'definition
                  '([(operation_definition (name) @font-lock-function-name-face)
                     ...
                     (fragment_name (name) @font-lock-function-name-face)
            ...
            (setq-local treesit-font-lock-settings
                        (append treesit-font-lock-settings
                                graphql-rules-with-override)))
          
          ;; 5. Extend font-lock feature list with GraphQL features
          (setq-local treesit-font-lock-feature-list
                      (cl-loop for host-features in treesit-font-lock-feature-list
                               for graphql-features in '((comment constant definition)
                                                         (keyword string number type variable)
                                                         (bracket delimiter)
                                                         nil)
                               collect (cl-union host-features graphql-features)))

          ;; 6. Set language-at-point function
          (setq-local treesit-language-at-point-function
                      (lambda (pos)
                        (let ((node (treesit-node-at pos host-lang)))
                          (if (and node
                                   (member (treesit-node-type node)
                                           '("string_fragment" "template_content"))
                                   (let ((call-expr (treesit-parent-until
                                                     node
                                                     (lambda (n) (equal (treesit-node-type n) "call_expression")))))
                                     (when call-expr
                                       (let ((func-node (treesit-node-child-by-field-name call-expr "function")))
                                         (and func-node
                                              (member (treesit-node-text func-node t)
                                                      '("gql" "graphql")))))))
                              'graphql
                            host-lang))))

          ;; 7. Recompute features and refresh
          (treesit-font-lock-recompute-features)
          (treesit-major-mode-setup))))

    ;; (add-hook 'tsx-ts-mode-hook #'my/setup-graphql-injection)
    ;; (add-hook 'typescript-ts-mode-hook #'my/setup-graphql-injection)
    )
  )
...
*** Preparation
:PROPERTIES:
:ID:       0b23dd80-d41d-43d6-a631-271cffd4eda6
:END:
...
(leaf mmm-mode
  ...
  :straight t
  :global-minor-mode t
  :custom (mmm-submode-decoration-level . 0) ;; Turn off background highlight
  :config
...
** mise
:PROPERTIES:
:ID:       4BB9CD89-ED00-4545-A8F9-75B1B507E2D7
:END:
