...
- [[#header][Header]]
  - [[#compilation-deny-packages][Compilation deny packages]]
...
2. Clone [[https://github.com/emacs-tree-sitter/elisp-tree-sitter][emacs-tree-sitter]]
   #+begin_src shell
     file core/tsc-dyn.dylib | grep -q arm64 || rm -f core/tsc-dyn.dylib
     grep -q LOCAL core/DYN-VERSION || printf LOCAL >core/DYN-VERSION
     grep -q DYN-VERSION bin/build && sed -e '/DYN-VERSION/d' bin/build >bin/build.tmp && mv bin/build.tmp bin/build && chmod +x bin/build || :
